...
const NuevoPrestamo = () => {
  ...
  const [formData, setFormData] = useState({
    cliente_id: '',
    monto: '',
    interes: '12', // Tasa por defecto
    plazo: '12', // Plazo por defecto
  ...
  const validateForm = () => {
    ...
    if (!formData.interes || parseFloat(formData.interes) <= 0) {
      errors.interes = 'Tasa de interés inválida';
    }
  ...
  const handleCreateClient = async () => {
    ...
    try {
      const response = await api.post('/cliente', newClientData);
  ...
  const calcularCuota = () => {
    if (!formData.monto || !formData.interes || !formData.plazo) return '0.00';
    
    const monto = parseFloat(formData.monto);
    const tasaSemanal = parseFloat(formData.interes) / 100 / 12;
    const plazo = parseInt(formData.plazo_semanal);
    
    if (monto <= 0 || plazo <= 0) return '0.00';
    
    const cuota = monto * (tasaSemanal * Math.pow(1 + tasaSemanal, plazo)) / 
                  (Math.pow(1 + tasaSemanal, plazo) - 1);
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          ...
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              <div>
                <label htmlFor="tasa_interes" className="block text-sm font-medium text-gray-700 mb-1">Tasa de interés anual (%) *</label>
                <div className="relative">
                  <input
                    type="number"
                    id="tasa_interes"
                ...
                {formErrors.tasa_interes && (
                  <p className="mt-1 text-sm text-red-600">{formErrors.interes}</p>
                )}
              </div>
              
              <div>
                <label htmlFor="plazo_meses" className="block text-sm font-medium text-gray-700 mb-1">Plazo (meses) *</label>
                <input
                  type="number"
                  id="plazo_meses"
                  name="plazo_meses"
                ...
                />
                {formErrors.plazo_semanas && (
                  <p className="mt-1 text-sm text-red-600">{formErrors.plazo_semanas}</p>
                )}
              </div>
              ...
              <div className="md:col-span-2">
                <div className="bg-blue-50 p-4 rounded-md">
                  <div className="flex justify-between items-center">
                    <div>
                      <p className="text-sm text-blue-800">Cuota mensual estimada</p>
                      <p className="text-xl font-bold text-blue-800">${calcularCuota()}</p>
                    </div>
                    <div className="text-right">
                      <p className="text-sm text-blue-800">Total a pagar</p>
                      <p className="text-xl font-bold text-blue-800">
                        ${(parseFloat(calcularCuota()) * parseInt(formData.plazo_semanas || 0)).toFixed(2)}
                      </p>
                    </div>
                  </div>
                </div>
            ...
            {showAval && (
              <div className="mt-4 space-y-4 p-4 bg-gray-50 rounded-lg">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    <label htmlFor="aval_apellido" className="block text-sm font-medium text-gray-700 mb-1">Apellido</label>
                    <input
                      type="text"
                      id="aval_apellido"
                      name="apellido"
                      value={avalData.apellidos}
                      onChange={handleAvalChange}
                      className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                    />
                  </div>
                  ...
                  <div>
                    <label htmlFor="aval_telefono" className="block text-sm font-medium text-gray-700 mb-1">Teléfono</label>
                    <input
                      type="tel"
                      id="aval_telefono"
                      name="telefono"
                  ...
                  <div className="md:col-span-2">
                    <label htmlFor="aval_direccion" className="block text-sm font-medium text-gray-700 mb-1">Dirección</label>
                    <textarea
                      id="aval_direccion"
                      name="direccion"
